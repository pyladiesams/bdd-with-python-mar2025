...
def step_impl(context):
    context.shipping_costs = {
        row["country"]: row for row in map(Row.as_dict, context.table)
    }


@given("tax rates")
def step_impl(context):
    context.tax_rates = {row["country"]: row for row in map(Row.as_dict, context.table)}
...
def step_impl(context, products: str):
    ...
    for item in items:
        context.user.basket.add(context.catalogue[item])
...
def step_impl(context):
    ...
    context.total_payable = user.checkout(
        tax_rates=context.tax_rates, shipping_costs=context.shipping_costs
    )
